...
export default function AdminShell({
  ...
  const UserMenu = (
    <div className="flex items-center gap-3 rounded-full border border-white/10 bg-white/5 py-1.5 pl-1.5 pr-2">
      <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#16785F] to-[#0B6B5E] text-xs font-bold text-white">
        {initials(name)}
      </span>
      <div className="hidden min-w-0 leading-tight sm:block">
        <p className="truncate text-sm font-semibold text-white">{name}</p>
        <p className="truncate text-xs capitalize text-[#16785F]/80">{profile?.role || "admin"}</p>
      </div>
      <button
  ...
  return (
    <div className="min-h-screen bg-[#07111f] text-slate-100">
      {/* ── Sidebar (desktop) ── */}
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-white/10 bg-[#1B1F20] lg:flex">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition ${
                  active
                    ? "bg-[#16785F]/15 text-[#16785F] ring-1 ring-[#16785F]/30"
      ...
      {/* ── Mobile top bar ── */}
      <header className="sticky top-0 z-40 flex items-center justify-between border-b border-white/10 bg-[#1B1F20] px-4 py-3 lg:hidden">
      ...
      <nav className="flex gap-1 overflow-x-auto border-b border-white/10 bg-[#1B1F20] px-2 py-2 lg:hidden">
        {NAV.map((item) => {
          const active = pathname === item.href;
          return (
            <Link
              ...
              className={`whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium ${
                active ? "bg-[#16785F]/15 text-[#16785F]" : "text-slate-400"
              }`}
            >
              {item.label}
            </Link>
      ...
      <div className="lg:pl-64">
        ...
        <header className="sticky top-0 z-30 hidden h-16 items-center justify-end border-b border-white/10 bg-[#07111f]/85 px-6 backdrop-blur lg:flex lg:px-10">
          {UserMenu}
        </header>
